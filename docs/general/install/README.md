...
```hcl
module "dynamodb" {
    source = "github.com/pbs/terraform-dynamodb-module?ref=x.y.z"
}
```
...
gh -R 'pbs/terraform-dynamodb-module' release download -p 'release.tar.gz' x.y.z
tar -xvf release.tar.gz -C 'terraform/modules/dynamodb'
rm -f release.tar.gz
```
...
```bash
git remote add -f dynamodb git@github.com:pbs/terraform-dynamodb-module.git
git subtree add --prefix terraform/modules/dynamodb dynamodb main --squash
```

If necessary, pin the repo version, rather than adding `main` as your subtree:

```bash
git subtree add --prefix terraform/modules/dynamodb dynamodb $REF --squash
```
